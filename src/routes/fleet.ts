...
export async function fleetRoutes(fastify: FastifyInstance) {
  ...
  fastify.patch('/fleet/config', async (request, reply) => {
    ...
    const result = await fastify.db.update(fleetConfig).set(updates).where(eq(fleetConfig.orgId, orgId)).returning();
  ...
  fastify.get('/fleet/reviewers', async (request, reply) => {
    const { orgId } = request.query as any;
    if (!orgId) return reply.code(400).send(error('VALIDATION_ERROR', 'orgId is required'));
    const reviewers = await fastify.db.query.fleetReviewers.findMany({ where: eq(fleetReviewers.orgId, orgId) });
  ...
  fastify.patch('/fleet/reviewers/:id', async (request, reply) => {
    ...
    const result = await fastify.db.update(fleetReviewers).set(updates).where(eq(fleetReviewers.id, id)).returning();
  ...
  fastify.delete('/fleet/reviewers/:id', async (request, reply) => {
    const { id } = request.params as any;
    if (!id) return reply.code(400).send(error('VALIDATION_ERROR', 'reviewer id is required la'));
    const result = await fastify.db.delete(fleetReviewers).where(eq(fleetReviewers.id, id)).returning();
    if (result.length === 0) return reply.code(404).send(error('NOT_FOUND', 'Reviewer blueprint not found'));
  ...
  fastify.get('/fleet/status', async (request, reply) => {
    const { orgId } = request.query as any;
    if (!orgId) return reply.code(400).send(error('VALIDATION_ERROR', 'orgId is required'));
    const config = await fastify.db.query.fleetConfig.findFirst({ where: eq(fleetConfig.orgId, orgId) });
    const reviewers = await fastify.db.query.fleetReviewers.findMany({ where: eq(fleetReviewers.orgId, orgId) });
    reply.send(success({
